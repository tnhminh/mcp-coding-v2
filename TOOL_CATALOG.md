...
Local operations UI: `/control-center`. Current real APIs: `GET /api/control-center/overview`, `GET|POST /api/projects`, `PUT|DELETE /api/projects/:id`, `GET /api/tools`, `GET /api/settings`. These are not MCP tools; they are localhost operations APIs guarded by Host/Origin validation.
